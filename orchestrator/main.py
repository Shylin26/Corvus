...
# In-memory incident state for MVP
# Phase D: replace with Postgres
...
class Orchestrator:
        ...
        while True:
            ...
            for plan_id, approval in list(pending_approvals.items()):
                if now > approval["expires_at"]:
                    log.warning(
                        "Approval expired for plan %s (incident %s)",
    ...
    def approve(self, plan_id: str, approver: str = "human") -> bool:
        ...
        asyncio.create_task(
            self._dispatch(approval["envelope"], approval["plan"])
        )
        return True
...
if __name__ == "__main__":
    asyncio.run(main())
EOFcat > orchestrator/main.py << 'EOF'
from __future__ import annotations
